...
void send_message(const char * remote_app_id, const char *name)
{
	...
	//char hello_message[20] = "Command clicked is = "; strcat(hello_message, name);
	bundle_add_str(b, "server", name);
	message_port_send_message(remote_app_id, "SAMPLE_PORTTT", b);
	bundle_free(b);
}
...
void message_port_cb(int local_port_id, const char *remote_app_id, const char *remote_port, bool trusted_remote_port, bundle *message) {
	...
	dlog_print(DLOG_INFO,"RECEIVE_NAME_CALLBACK","Messagefrom%s, name:%s", remote_app_id, name);
...
bool service_app_create(void *data)
{
	...
	//To open an interface invoked by other apps
	int local_port_id = message_port_register_local_port("SAMPLE_PORTTT", message_port_cb, NULL);
	if (local_port_id < 0) {
